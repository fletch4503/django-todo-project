...
class pwp_exch_model:
    ...
    def __str__(self):  # Информируем о названии объекта
    ...
    def __init__(self):  # Подключаемся к Exchange-серверу и проверяем подключение
        ...
        try:
            self.conf_exchange = Configuration(
                server=exch_serverurl, retry_policy=FaultTolerance(max_wait=3600),
                credentials=self.credents_project,
        ...
        # print("Объекты ews_items ews_exch: %s", str(self))
    ...
    #                 try:
    #                     print('create_attachement_list -> Сообщение:',
    #                     attachment.item.subject, attachment.item.body)
    #                 except:
    #                     pass
    #     return files_attach
    #
